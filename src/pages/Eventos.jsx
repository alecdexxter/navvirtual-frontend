...
function Eventos() {
    ...
    return (
      ...
      <span className="font-mono text-xs text-senal tracking-widest uppercase">
        ◣ Hola, {usuario?.nombre}
      </span>
            ...
            {eventos.length === 0 && (
                <p className="text-tinta/60">No hay eventos por ahora.</p>
            )}
            ...
            <div className="grid sm:grid-cols-2 gap-5">
                {eventos.map((ev) => (
                    <Link
                        key={ev.id}
                        to={`/evento/${ev.id}`}
                        className="group block bg-superficie rounded-2xl p-6 hover:bg-tinta transition-colors"
                    >
