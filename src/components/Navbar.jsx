...
function Navbar() {
    ...
    return (
        <>
            <header className="flex items-center justify-between px-6 py-4 bg-tinta text-fondo">
                <button
                    onClick={() => setAbierto(true)}
                    className="flex flex-col gap-1.5 w-6 group"
                    aria-label="Abrir menú"
                >
                    <span className="h-0.5 w-full bg-fondo group-hover:bg-ambar transition-colors" />
                    <span className="h-0.5 w-full bg-fondo group-hover:bg-ambar transition-colors" />
                    <span className="h-0.5 w-4 bg-fondo group-hover:bg-ambar transition-colors" />
                </button>
                ...
                {usuario ? (
                    <span className="font-mono text-xs text-superficie">{usuario.nombre}</span>
                ) : (
                    <span className="w-6" />
                )}
            </header>
            ...
            <div
                className={`fixed inset-0 z-50 transition-opacity ${abierto ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
            >
                <div
                ...
                />
                <nav
                    className={`absolute left-0 top-0 h-full w-72 bg-fondo px-6 py-6 flex flex-col gap-1 transition-transform duration-300 ${abierto ? 'translate-x-0' : '-translate-x-full'}`}
                >
                    <button
                        onClick={() => setAbierto(false)}
                        className="self-end text-2xl leading-none mb-4"
...
function MenuItem({ label, onClick }) {
    return (
        <button
            onClick={onClick}
            className="text-left font-display text-2xl py-3 border-b border-superficie flex items-center gap-3 hover:text-senal transition-colors"
        >
